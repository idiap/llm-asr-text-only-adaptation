...
```bash
TARGET_DOMAIN=banking bash scripts/table2-ma-et-al/run_all.sh
TARGET_DOMAIN=insurance bash scripts/table2-ma-et-al/run_all.sh
```
...
```bash
TARGET_DOMAIN=banking bash scripts/table2-ma-et-al/decode_all.sh
TARGET_DOMAIN=insurance bash scripts/table2-ma-et-al/decode_all.sh
```

Each decoded checkpoint writes its own `WER.txt`:

```
exp/table2/TOA/prompt_llama/<domain>_ma_et_al/<checkpoint>/WER.txt
```
