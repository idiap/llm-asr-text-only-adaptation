...
# Adapt the LLM with LoRA. The speech encoder, the projector and the LLM base weights stay
# frozen; batches mix source audio, projector-induced noise, synthetic noise and target text
# according to tau/sigma (Section 2.2 of the paper).
projector_ckpt_path=$RUN_DIR/exp/$EXPERIMENT_NAME/base/${PROMPT}/${BASE_CKPT_FOLDER}
output_dir=$RUN_DIR/exp/$EXPERIMENT_NAME/${PROMPT_METHOD}/${PROMPT}/${EXPERIMENT_SETUP_NAME}

[ ! -d $output_dir ] && mkdir -p $output_dir

# Ma et al. only: reuse the soft-prompt embeddings learned in the previous stage.
soft_prompt_args=""
if [ "$USE_SOFT_PROMPT_EMB" = "true" ]; then
    pemb_path=$output_dir/p-tuning/${TOA_CKPT_FOLDER}/adapter_model.safetensors
    soft_prompt_args="++train_config.prompt_embeddings_path=$pemb_path"
fi
...
++train_config.use_text_only_adaptation=$TOA_ENABLED \
++train_config.text_only_adaptation_config.tau_a=$TOA_TAU_A \
++train_config.text_only_adaptation_config.tau_t=$TOA_TAU_T \
++train_config.text_only_adaptation_config.sigma_a=$TOA_SIGMA_A \
++train_config.text_only_adaptation_config.sigma_ta=$TOA_SIGMA_TA \
++train_config.text_only_adaptation_config.sigma_t=$TOA_SIGMA_T \
++train_config.text_only_adaptation_config.noise_type=$TOA_NOISE_TYPE \
...
cmd="sbatch $SLURM_ACCOUNT_ARG --job-name ft-$EXPERIMENT_NAME-$EXPERIMENT_SETUP_NAME --partition=$SLURM_PARTITION --gpus=${SLURM_GPU_TYPE}:${SLURM_NUM_GPUS} --ntasks=1 --nodes=1 $SLURM_QOS_ARG"
