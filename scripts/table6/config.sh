...
#
# Table 6: ablation over the noise function used to build the LLM input (DefinedAI targets B and I).
#
...
# naive  -> the paper's noise(): random character substitution followed by duplication
# random -> characters drawn uniformly from a-z/A-Z/space, matching the transcript length
# echo   -> the clean transcript itself
...
DEFAULT_EXPERIMENT_NAME=${DEFAULT_EXPERIMENT_NAME:-table6}
if [ "$TOA_ENABLED" = "false" ]; then
    ...
    if [ "$TOA_NOISE_TYPE" != "naive" ]; then
        DEFAULT_EXPERIMENT_SETUP_NAME="${DEFAULT_EXPERIMENT_SETUP_NAME}_noise_${TOA_NOISE_TYPE}"
    fi
fi
