...
# Table 2: Fang et al. [14] baseline
...
```bash
TARGET_DOMAIN=banking bash scripts/table2-fang-et-al/run_all.sh
TARGET_DOMAIN=insurance bash scripts/table2-fang-et-al/run_all.sh
```
...
```bash
TARGET_DOMAIN=banking bash scripts/table2-fang-et-al/decode_all.sh
TARGET_DOMAIN=insurance bash scripts/table2-fang-et-al/decode_all.sh
```

Each decoded checkpoint writes its own `WER.txt`:

```
exp/table2/TOA/prompt_llama/<domain>_fang_et_al/<checkpoint>/WER.txt
```

Report the lowest WER across checkpoints, which is the number that appears in Table 2.
