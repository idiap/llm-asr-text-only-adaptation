...
# Based on https://github.com/huggingface/peft/blob/main/src/peft/peft_model.py
# with some refactor
#
# Modifications by Sergio Burdisso (Idiap Research Institute © 2025):
#   - Added support for P_PROJECTOR as a new PEFT type
...
PEFT_TYPE_TO_MODEL_MAPPING = {
    ...
    PeftType.P_PROJECTOR: PromptEncoder,
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _setup_prompt_encoder(self, adapter_name: str):
        ...
        elif config.peft_type in (PeftType.P_TUNING, PeftType.P_PROJECTOR):
    ...
    def get_prompt_inputs_embeds(
        ...
        """
        peft_config = self.active_peft_config
        has_virtual_tokens = peft_config.peft_type in (PeftType.P_TUNING, PeftType.P_PROJECTOR) and peft_config.virtual_token_id is not None
    ...
    def forward(
        ...
        **kwargs,
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if kwargs.get("token_type_ids", None) is not None and not has_virtual_tokens:
...
class PeftModelForCausalLM(PeftModel):
    ...
    def prepare_inputs_for_generation(self, *args, task_ids: Optional[torch.Tensor] = None, inputs_embeds=None, **kwargs):
        kwargs["inputs_embeds"] = inputs_embeds
        cache_position = False
        if "cache_position" not in kwargs and inputs_embeds is not None:
            # To avoid bug in transformers
            # Fixed in later versions after in this commit below:
            # https://github.com/huggingface/transformers/commit/c8524aeb07f370195733f610760236d22e88cb61#diff-26783ca033d92b4ce2e01eb691dbf5b05dd972b0cbfdc69fc726cf77a9dcb011
            # Related to the following PR: https://github.com/huggingface/transformers/pull/38635
            cache_position = True
            kwargs["cache_position"] = torch.arange(0, inputs_embeds.shape[1], dtype=torch.long, device=inputs_embeds.device)
        peft_config = self.active_peft_config
        has_virtual_tokens = peft_config.peft_type in (PeftType.P_TUNING, PeftType.P_PROJECTOR) and peft_config.virtual_token_id is not None
        ...
        if "input_ids" not in kwargs:
            return model_kwargs
        if has_virtual_tokens:
            model_kwargs["input_ids"] = kwargs["input_ids"]
        ...
        if peft_config.is_prompt_learning:
            if uses_cache and "past_key_values" in model_kwargs and model_kwargs["past_key_values"] is not None:
            ...
            input_key = "input_ids" if "input_ids" in model_kwargs else "inputs_embeds"
            batch_size = model_kwargs[input_key].shape[0]
            if model_kwargs.get("attention_mask", None) is not None and not has_virtual_tokens:
                input_key = "input_ids" if "input_ids" in model_kwargs else "inputs_embeds"
                size = batch_size, peft_config.num_virtual_tokens
                prefix_attention_mask = torch.ones(size).to(model_kwargs[input_key].device)
            ...
            requires_prompt_injection = ("past_key_values" not in model_kwargs or model_kwargs["past_key_values"] is None) or (
            ...
            if requires_prompt_injection and peft_config.peft_type == PeftType.PREFIX_TUNING:
                ...
                if has_virtual_tokens:
                    check_ptuning_input_is_valid(model_kwargs["input_ids"], model_kwargs["inputs_embeds"], peft_config)
                    virtual_tokens_ixs = (model_kwargs["input_ids"] == peft_config.virtual_token_id).nonzero(as_tuple=True)

                if model_kwargs["inputs_embeds"] is None:
                    if has_virtual_tokens: model_kwargs["input_ids"][virtual_tokens_ixs] = 0
                    model_kwargs["inputs_embeds"] = self.word_embeddings(model_kwargs["input_ids"])
                    if has_virtual_tokens: model_kwargs["input_ids"][virtual_tokens_ixs] = peft_config.virtual_token_id
                prompts = self.get_prompt(batch_size=_get_batch_size(model_kwargs["input_ids"], model_kwargs["inputs_embeds"]),
                                          task_ids=task_ids).to(model_kwargs["inputs_embeds"].dtype)
                prompts = prompts[:, : peft_config.num_virtual_tokens]  # Getting rid of decoder prompt embeddings (in case of encoder-decoder)
                if has_virtual_tokens:
                    model_kwargs["inputs_embeds"][virtual_tokens_ixs] = prompts.reshape(-1, prompts.shape[-1])
                else:
                    model_kwargs["inputs_embeds"] = torch.cat((prompts, model_kwargs["inputs_embeds"]), dim=1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if decoder_attention_mask is not None:
            ...
            if peft_config.peft_type not in (PeftType.PROMPT_TUNING, PeftType.P_TUNING, PeftType.P_PROJECTOR):
        ...
        elif peft_config.peft_type in (PeftType.PROMPT_TUNING, PeftType.P_TUNING, PeftType.P_PROJECTOR):
            if attention_mask is not None and not has_virtual_tokens:
                # concat prompt attention mask
                prefix_attention_mask = torch.ones(batch_size, peft_config.num_virtual_tokens).to(
