...
#   - Added text-only adaptation: auxiliary source-domain dataloader and tau-based batch split
...
def main(kwargs: DictConfig):
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.manual_seed_all(train_config.seed) # for multi-GPU
    ...
    dataset_train_aux = None
    if train_config.use_text_only_adaptation and dataset_config.train_aux_data_path is not None:
        dataset_train_aux = get_preprocessed_dataset(
            tokenizer,
            processor,
            dataset_config,
            split="train",
    ...
    if not (train_config.enable_fsdp or train_config.enable_ddp) or rank == 0:
        ...
        if train_config.use_text_only_adaptation and dataset_config.train_aux_data_path is not None:
            logger.info(f"--> Auxiliary Training Set Length = {len(dataset_train_aux)}")
...
# Create DataLoaders for the training and validation dataset
    ...
    if dataset_train_aux is None:
        ...
        original_batch_size =  train_config.batch_size_training 
        tau_t = train_config.text_only_adaptation_config.tau_t
        tau_a = train_config.text_only_adaptation_config.tau_a
        tau = tau_t + tau_a

        # Calculate auxiliary batch size as (1 - tau) * original_batch_size
        aux_batch_size = int((1 - tau) * original_batch_size)
        if aux_batch_size <= 0:
            logger.info(f"Calculated auxiliary batch size is 0 due to tau being {tau}. "
                        "Therefore, NO auxiliary data will be used for text-only adaptation.")

        train_dl_kwargs = get_dataloader_kwargs(
            train_config, dataset_train, tokenizer, "train", batch_size=original_batch_size - aux_batch_size
        )
        ...
        # Restoring the original batch size just in case
        train_dl_kwargs['batch_size'] = original_batch_size
        if aux_batch_size > 0:
            # Getting dataloader kwargs for auxiliary dataset - this fixes the size of the auxiliary dataset
            train_aux_dl_kwargs = get_dataloader_kwargs(
                train_config, dataset_train_aux, tokenizer, "train", batch_size=aux_batch_size
            )

            train_aux_dataloader = torch.utils.data.DataLoader(
                dataset_train_aux,
                num_workers=train_config.num_workers_dataloader,
                pin_memory=True,
                **train_aux_dl_kwargs,
