...
def save_model_checkpoint_peft(model, optimizer, rank, cfg, checkpoint_name="checkpoint", save_trainable_only=True):
    ...
    if cfg.use_peft:
        model.llm.save_pretrained(os.path.dirname(save_full_path))
        logger.info(f"saving full peft model at {os.path.dirname(save_full_path)}")
...
def save_model_checkpoint_peft_full_shard(model, optimizer, rank, cfg, epoch=0):
    ...
    if rank == 0:
        ...
        if cfg.use_peft:
            ...
            logger.info(f"saving full peft model at {save_dir}")
